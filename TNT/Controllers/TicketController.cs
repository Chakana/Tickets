    ...
    public class TicketController : Controller
    {
        ...
        public JsonResult ComprarTicketsMultiple(List<req_tickets_multiple> req, string metodo)
        {
            ...
            try
            {
                ...
                {
                    var sector = db.sectores.Find(Int32.Parse(ticket_reserva.id_sector));
                    var evento = sector.Eventos;
                    int id_evento = evento.id;
                    ...
                    if (tickets_comprados_usuario.Count() > 5)
                    {
                        ...
                        respuesta.mensaje = "Usted ya compro 5 tickets para este evento, no puede comprar más";
                        return Json(respuesta);
                    }
        ...
        public ActionResult VerificarDetalleCompra(List<req_tickets_multiple> req, string metodo)
        {
            ...
            ViewBag.Nombre = datos_usuario.Personas.FirstOrDefault().nombre + " " + datos_usuario.Personas.FirstOrDefault().apellidos;
        ...
        public void CompraMultiplesTicketMismoSector(Ticket ticket)
        {
            ...
            try
            {
                ...
                if (!envio)
                {
                    ...
                    relegado.nit_empresa = datos_usuario.Personas.First().cedula_identidad.Trim();
        ...
        public void CompraIndividualTicket(Ticket ticket, int cantidad = 1)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (!envio)
                {
                    ...
                    relegado.email_usuario = User.Identity.Name;
                    relegado.fecha_hora = DateTime.Now;
                    relegado.nit_empresa = datos_usuario_actual.cedula_identidad.Trim();
                    relegado.nit_usuario = ticket.nit_usuario;
                    relegado.nombre_empresa = evento.Empresas.nombre_empresa;
                    relegado.nombre_evento = evento.nombre_evento;
                    relegado.nombre_usuario = User.Identity.Name;
                    relegado.pendiente = true;
                    relegado.total_comision = comision.monto_comision;
                    db.EnviosRelegadosSINTESIS.Add(relegado);
                    db.SaveChanges();
                    helpers.Helpers.EnviarMail("admin@tnt.com", "admin", "eisenob@gmail.com", "admin", "CODIGO RECAUDACION NO ENVIADO SINTESIS - PENDIENTE", "<h1>Codigo de recaudacion no enviado y pendiente de envio</h1><h2>" + codigo_recaudacion + "</h2>", null);
                ...
                try
                {
                    ...
                    ViewBag.message = "compra exitosa, por favor imprima el voucher haciendo clic en el icono de imprimir";
                    Session.Add("codigo_recaudacion", codigo_recaudacion);
                    Session.Add("monto_pagar", (costo_total + comision.monto_comision));
